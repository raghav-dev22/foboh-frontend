...
import { OrderingContactSchema } from '../schemas';
...
function OrderingContact() {
  const { values, errors, handleBlur, handleChange, handleSubmit, touched } =
    useFormik({
      initialValues: initialValues,
      validationSchema: OrderingContactSchema,
      onSubmit: (values) => {
    ...
    <form className="w-full max-w-lg" onSubmit={handleSubmit}>
        ...
        <div className="w-full md:w-1/2 px-3 relative">
          ...
          />
            {errors.firstName && touched.firstName && (
                  <p className="mt-2 mb-2 text-red-500 text-xs	font-normal	">{errors.firstName}</p>
                )}
                {errors.firstName && touched.firstName && (
        ...
        <div className="w-full md:w-1/2 px-3 relative">
          ...
          />
            {errors.lastName && touched.lastName && (
                  <p className="mt-2 mb-2 text-red-500 text-xs	font-normal">{errors.lastName}</p>
                )}
                {errors.lastName && touched.lastName && (
                  <ErrorOutlineIcon className="absolute text-red-500 top-[47px] right-5 transition-all duration-[0.3s]" />
                )}
        ...
        <div className="w-full px-3">
          ...
          />
            {errors.email && touched.email && (
                  <p className="mt-2 mb-2 text-red-500 text-xs	font-normal">{errors.email}</p>
                )}
                {errors.email && touched.email && (
                  <ErrorOutlineIcon className="absolute text-red-500 top-[47px] right-5 transition-all duration-[0.3s]" />
                )}
      ...
      <div className="flex flex-wrap -mx-3 mb-5 relative">
        <div className="w-full px-3">
          ...
          />
            {errors.email && touched.email && (
                  <p className="mt-2 mb-2 text-red-500 text-xs	font-normal">{errors.mobile}</p>
                )}
